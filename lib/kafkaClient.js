  ...
  socket.on('connect', function () {
    ...
    if (lastError) {
      ...
      self.initializeBroker(brokerWrapper, function (error) {
        ...
        logger.debug(`Initialized broker 1, ${readyEventName}`);
      ...
      self.initializeBroker(brokerWrapper, function (error) {
        ...
        logger.debug(`Initialized broker 2, ${readyEventName}`);
  ...
  function retry (s) {
    ...
    s.retryTimer = setTimeout(function () {
      ...
      if (!self.options.reconnectOnIdle && brokerWrapper.isIdle()) {
        ...
        logger.debug(`self options: ${JSON.stringify(self.options)}`);
        logger.debug(`broker wraper: ${brokerWrapper.isIdle()}`);
        s.closing = true;
